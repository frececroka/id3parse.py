...
class TestID3Header(unittest.TestCase):
	...
	def test_header_creation_fails_if_wrong_test_identifier_is_present(self):
		with self.assertRaises(ID3IllegalFormatError):
			ID3Header.from_byte_array(b'ID4\x04\x00\x00\x00\x00\x08\x00')

	def test_header_creation_fails_if_tag_has_higher_major_version(self):
		with self.assertRaises(ID3UnsupportedVersionError):
			ID3Header.from_byte_array(b'ID3\x05\x00\x00\x00\x00\x08\x00')

	def test_header_creation_succeeds_if_tag_has_lower_major_version(self):
		ID3Header.from_byte_array(b'ID3\x03\x00\x00\x00\x00\x08\x00')

	def test_header_contains_correct_flags(self):
		header = ID3Header.from_byte_array(b'ID3\x04\x00\x50\x00\x00\x08\x00')
	...
	def test_missing_id3_tag(self):
		with self.assertRaises(ID3IllegalFormatError):
			header = ID3Header.from_byte_array(b'\xff\xf3\x23\x04\x00\x50\x00\x00\x08\x00')
...
class TestID3(unittest.TestCase):
	...
	def test_average_case_deserialization(self):
		id3 = ID3.from_byte_array(TestID3.test_average_case_data)
		self.verify_average_case_tag(id3)

	def test_average_case_serialization(self):
		id3 = ID3.from_byte_array(TestID3.test_average_case_data)
		id3 = ID3.from_byte_array(id3.serialize())
		self.verify_average_case_tag(id3)
	...
	def test_id3_version_3_frame_size(self):
		...
		id3 = ID3.from_byte_array(byte_array)
	...
	def test_serialization_with_extended_header(self):
		...
		with self.assertRaises(ID3UnsupportedFeatureError):
			id3.serialize()

	def test_write_to_file_where_file_contains_bigger_tag(self):
		path = self.make_mp3_with_tag_and_padding()

		id3 = ID3.from_file(path)
		id3.add_frame(ID3TextFrame.from_scratch('TIT2', 'Welcome'))
		id3.to_file()
...
class TestID3Frame(unittest.TestCase):
	...
	def test_all_flags_unset(self):
		...
		frame = ID3Frame.from_byte_array(byte_array)
	...
	def test_unsupported_feature_compression(self):
		with self.assertRaises(ID3UnsupportedFeatureError):
			...
			ID3Frame.from_byte_array(byte_array)
...
class TestID3TextFrame(unittest.TestCase):
	...
	def test_serialization(self):
		frame = ID3TextFrame.from_scratch('TIT2', 'Caught Me Thinking')
	...
	def test_creation_from_scratch_with_illegal_frame_name(self):
		with self.assertRaises(ValueError):
			ID3TextFrame.from_scratch('ABCD', 'Caught Me Thinking')
...
class TestID3CommentFrame(unittest.TestCase):
	...
	def test_serialization(self):
		frame = ID3CommentFrame.from_scratch('deu', 'a short description', 'the actual comment')
...
class TestID3PopularimeterFrame(unittest.TestCase):
	...
	def test_serialization(self):
		frame = ID3PopularimeterFrame.from_scratch('user@localhost', 128, 30)
	...
	def test_serialization_of_large_play_counter(self):
		frame = ID3PopularimeterFrame.from_scratch('user@localhost', 128, 9999999999)

		self.assertEqual(b'POPM\x00\x00\x00\x15\x00\x00user@localhost\x00\x80\x02\x54\x0b\xe3\xff', frame.serialize())

	def test_deserialization(self):
		frame = ID3Frame.from_byte_array(b'POPM\x00\x00\x00\x14\x00\x00user@localhost\x00\x80\x00\x00\x00\x1e')
	...
	def test_deserialization_of_large_play_counter(self):
		frame = ID3Frame.from_byte_array(b'POPM\x00\x00\x00\x15\x00\x00user@localhost\x00\x80\x02\x54\x0b\xe3\xff')
...
class TestID3PlayCounterFrame(unittest.TestCase):
	...
	def test_serialization(self):
		frame = ID3PlayCounterFrame.from_scratch(30)

		self.assertEqual(b'PCNT\x00\x00\x00\x04\x00\x00\x00\x00\x00\x1e', frame.serialize())

	def test_deserialization(self):
		frame = ID3Frame.from_byte_array(b'PCNT\x00\x00\x00\x04\x00\x00\x00\x00\x00\x1e')
		self.assertEqual(30, frame.play_counter)

	def test_serialization_of_large_play_counter(self):
		frame = ID3PlayCounterFrame.from_scratch(9999999999)

		self.assertEqual(b'PCNT\x00\x00\x00\x05\x00\x00\x02\x54\x0b\xe3\xff', frame.serialize())

	def test_deserialization_of_large_play_counter(self):
		frame = ID3Frame.from_byte_array(b'PCNT\x00\x00\x00\x05\x00\x00\x02\x54\x0b\xe3\xff')
		self.assertEqual(9999999999, frame.play_counter)
...
class TestID3PictureFrame(unittest.TestCase):
	...
	def test_serialization(self):
		frame = ID3PictureFrame.from_scratch('image/jpeg', ID3PictureTypes.BRIGHT_COLOURED_FISH, 'A bright coloured fish', b'....the...picture....')
		self.assertEqual(b'APIC\x00\x00\x009\x00\x00\x03image/jpeg\x00\x11A bright coloured fish\x00....the...picture....', frame.serialize())

	def test_deserialization(self):
		frame = ID3Frame.from_byte_array(b'APIC\x00\x00\x009\x00\x00\x03image/jpeg\x00\x11A bright coloured fish\x00....the...picture....')

		self.assertEqual('image/jpeg', frame.mime_type)
		self.assertEqual(ID3PictureTypes.BRIGHT_COLOURED_FISH, frame.picture_type)
...
class TestID3FrameHeader(unittest.TestCase):
	...
	def test_base_frame_header(self):
		...
		frame_header = ID3FrameHeader.from_byte_array(byte_array)
...
class TestSetAndGetFlag(unittest.TestCase):
	...
	def test_pack_and_get_flags(self):
		flags_byte = pack_flags((True, False, True, True, False, False, False, True))[0]

		self.assertTrue(get_flag(flags_byte, 7))
		self.assertFalse(get_flag(flags_byte, 6))
		self.assertTrue(get_flag(flags_byte, 5))
		self.assertTrue(get_flag(flags_byte, 4))
		self.assertFalse(get_flag(flags_byte, 3))
		self.assertFalse(get_flag(flags_byte, 2))
		self.assertFalse(get_flag(flags_byte, 1))
		self.assertTrue(get_flag(flags_byte, 0))
...
class TestTerminatedStrings(unittest.TestCase):
	...
	def test_extract_terrminated_string_with_single_zero_as_terminator(self):
		...
		string = extract_terminated_string(terminated_string, b'\x00')
	...
	def test_extract_terrminated_string_with_double_zero_as_terminator(self):
		...
		encoded_string = extract_terminated_string(terminated_string, b'\x00\x00')
	...
	def test_extract_terminated_string_with_missing_terminator(self):
		with self.assertRaises(ValueError):
			...
			extract_terminated_string(terminated_string, b'\x00\x00')
